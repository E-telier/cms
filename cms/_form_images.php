<?php
	...
	if (isset($_POST['submit_check'])) {
		...
		for ($l=0;$l<$nb_lang;$l++) {
			...
			if (empty($image) == false) {
				...
				if (empty($name)) { 
					...
					if (!mb_detect_encoding($name, 'UTF-8', true)) { $name = utf8_encode($name); } // DETECT WINDOWS FILE						
				...
				if (array_search($name, $done_images)===false) {
					...
					$img_result = eFile::add_image($file_datas, $folder, $name, $extension, $width, $height, $rotation, $resize);
					
					if (is_array($img_result)===true) {
						$imgMod = true;
				...
				$dest_size = eFile::get_size_within_image_ratio($full_size, $_POST[$temp_lang.'_width'], $_POST[$temp_lang.'_height']);
				$width = $dest_size[0];
				$height = $dest_size[1];
			...
			if ($imgMod == true) {
				...
				if ($addmod==0) {
					...
					(global_ref, name, folder, extension, description, width, height, weight, resize, align, openbig, slideshow_pos, gallery_pos, lastedit) 
						VALUES 
					...
					$rq = "UPDATE ".eParams::$prefix.'_'.$temp_lang."$table SET name='".eMain::$sql->protect_sql($name)."', folder='".eMain::$sql->protect_sql($folder)."', extension='".eMain::$sql->protect_sql($extension)."', description='".eMain::$sql->protect_sql($_POST[$temp_lang.'_description'])."', width=".$width.", height=".$height.", weight=".$weight.", resize='".eMain::$sql->protect_sql($_POST[$temp_lang.'_resize'])."', align='".eMain::$sql->protect_sql($_POST[$temp_lang.'_align'])."', openbig=".$openbig.", slideshow_pos=".$slideshow_pos.", gallery_pos=".$gallery_pos.", lastedit='".date('YmdHis')."' WHERE global_ref=".$addmod.";";
				}
								
				if (!eMain::$sql->sql_query($rq)) {
					$error_msg = "ERROR CMS002 : Unable to process the query\n$rq\n".eMain::$sql->get_error();
					eMain::add_error($error_msg);
					
					$ajax_result .= $error_msg."\n";
					
				} else {
					echo '<div class="success">Données '.$temp_lang.' correctement enregistrées !</div>';
					
					$success_msg = "lang=".$temp_lang . $ajax_items_separator;
					$success_msg .= "name=".$name . $ajax_items_separator;
					$success_msg .= "extension=".$extension . $ajax_items_separator;
					$success_msg .= "description=".$_POST[$temp_lang.'_description'] . $ajax_items_separator;
					$success_msg .= "folder=".$folder . $ajax_items_separator;
					$success_msg .= "width=".$width . $ajax_items_separator;
					$success_msg .= "height=".$height . $ajax_items_separator;
					$success_msg .= "align=".$_POST[$temp_lang.'_align'] . $ajax_items_separator;
					$success_msg .= "resize=".$_POST[$temp_lang.'_resize'] . $ajax_items_separator;		
					
					$ajax_result .= $success_msg."\n";						
		...
		if (isset($_POST['ajax'])) {
			echo "<!-- AJAX RESULT -->".$ajax_result."<!-- END OF AJAX RESULT -->";				
		}
	...
	if ($addmod==0) {
		...
		$title = 'Modification de l\'image référence "'.$values[eLang::get_lang()]['name'].'"';
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('input[type="text"]').each(function() {
				let tSize = $(this).attr('size');				
				if (typeof tSize !== 'undefined' && tSize!==false) {
					$(this).addClass('auto_width');
				}
			});
		});
		...
		function asyncFormSubmitted(tResult) {
			...
			for (var i=0;i<tResult.length;i++) {						
				var tValues = tResult[i].split('<?php echo $ajax_items_separator; ?>');
			...
			$('select[id$=_addimage]').each(function() {
				let tName = $(this).attr('id');
				let tLang = tName.substring(0, 2);						
				let tWysiwygName = tName.replace('_addimage', '');

				if (tLangValues.hasOwnProperty(tLang)===false) {
					console.warn(tLang, tLangValues);
					alert('ERROR: Could not load image information');
					return false;
				}
				...
				tImgList['folder'] = tLangValues[tLang]['folder'];
				
				myWysiwygs[tWysiwygName].imgList[tLangValues[tLang]['name']] = tImgList;
				
			});
			
			// ADD IMAGE OPTION 
			$('select.img_select').each(function() {
				let tLang = ($(this).attr('id')).split('_')[0];				
				let tName = tLangValues[tLang]['name'];
				let tFolder = (tLangValues[tLang]['folder']=='') ? '' : tLangValues[tLang]['folder']+'/';
				let tOption = '<option value="'+tName+'" class="img_option" style="background-image:url(../images/'+tFolder+tName+'.'+tLangValues[tLang]['extension']+');">'+tName+'</option>';		
				$(this).children('option:eq(0)').after(tOption);
			
			});
			
			// RESET CUSTOM SELECT //
			$('div.custom_select').remove();
			mySelectsManager = new eCustomSelectsManager();
								
			// CLOSE POPUP //
			ePopup.closePopup($('.popup form')[0]);
...
?>	
	<div id="form_<?php echo $temp_lang; ?>" class="form_lang">
		...
		<input type="file" name="<?php echo $temp_lang; ?>_image" accept="image/*" />
		<br /><br />
		
		<b>Nom : (Vide = défaut = nom du fichier d'origine)</b><br />
		<input type="text" value="<?php echo $values[$temp_lang]['name']; ?>" name="<?php echo $temp_lang; ?>_name" size="48" />
		<br /><br />
...
?>
		...
		<h2 id="details_title" onclick="$('#details').css('display', 'block');">Détails</h2>
		...
		<div id="details">
			...
			<input type="radio" name="<?php echo $temp_lang.'_resize'; ?>" id="<?php echo $temp_lang.'_resize_'.'crop'; ?>" value="crop" <?php if ($values[$temp_lang]['resize']=='crop') { ?>checked="checked"<?php } ?> /> <label for="<?php echo $temp_lang.'_resize_'.'crop'; ?>">Couper surplus</label>
			<input type="radio" name="<?php echo $temp_lang.'_resize'; ?>" id="<?php echo $temp_lang.'_resize_'.'deform'; ?>" value="deform" <?php if ($values[$temp_lang]['resize']=='deform') { ?>checked="checked"<?php } ?> /> <label for="<?php echo $temp_lang.'_resize_'.'deform'; ?>">Déformer</label>
			<input type="radio" name="<?php echo $temp_lang.'_resize'; ?>" id="<?php echo $temp_lang.'_resize_'.'full'; ?>" value="full" <?php if ($values[$temp_lang]['resize']=='full') { ?>checked="checked"<?php } ?> /> <label for="<?php echo $temp_lang.'_resize_'.'full'; ?>">Garder tout</label>
			...
			<br /><br />
			<b>Position dans le slideshow : </b>
			<input type="text" size="1" value="<?php echo $values[$temp_lang]['slideshow_pos']; ?>" name="<?php echo $temp_lang; ?>_slideshow_pos" class="auto_width" /> (0 = défaut = absent du slideshow)
			<br /><br />
			<b>Position dans la galerie : </b>
			<input type="text" size="1" value="<?php echo $values[$temp_lang]['gallery_pos']; ?>" name="<?php echo $temp_lang; ?>_gallery_pos" onchange="return false; if (this.value>0) { updateWidth(this, 200, 150); }" class="auto_width" /> (0 = défaut = absent de la galerie)
			<br />
			<?php if ($addmod!=0) { 
				echo "<br /><b>Aperçu : </b><br />";
				...
				if ($folder!='') { $folder .= '/'; }
				echo '<div class="img img_center center"><img src="'.$root_folder.$folder.$values[$temp_lang]['name'].'.'.$values[$temp_lang]['extension'].'?d='.date("YmdHis").'" width="'.$width.'" height="'.$height.'" /></div>'; } 
			?>
