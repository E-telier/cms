	<?php
		...
		if (isset($_POST['submit_check'])) {
			...
			for ($l=0;$l<$nb_lang;$l++) {
				...
				if (!eMain::$sql->sql_query($rq)) {
					echo "<div class=\"error\">ERROR CMS002 : Unable to process the query<br />$rq<br />".eMain::$sql->get_error()."</div>";
				} else {
		...
		if ($addmod==0) {
			...
			$rq = "SELECT id FROM ".eParams::$prefix.'_'.eLang::get_lang()."$table";
			...
			$title = 'Modification du text-block référence "'.$values[eLang::get_lang()]['reference'].'"';
...
?>
		...
		<?php 
			$wysiwyg_params = array(
				'content' => $values[$temp_lang]['content'],
				'name' => $temp_lang.'_'.$base_wysiwyg_name
			);
