...
$dictionnary['fr']['r�sultats'] = "r�sultats";		
$dictionnary['fr']['mots cl�s'] = "mots cl�s";	
$dictionnary['fr']['pour'] = "pour";

$dictionnary['en']['r�sultats'] = "results";
$dictionnary['en']['mots cl�s'] = "keywords";
$dictionnary['en']['pour'] = "for";

$traductions = $dictionnary[eLang::get_lang()];
...
if (strlen($search_string)>0) {
	...
	$table=$localized_sql_prefix."_cms_pages";
	...
	function getSearchValue($chaine) {
		...
		for ($mot=0;$mot<$nbreMots;$mot++) {
			...
			// Mot Interdit ? //
			$motsInterdits = array("le", "la", "les", "de", "du", "des", "�", "et", "ce", "se", "dans");
			$motInterdit = false;
	...
	function boldKeywords ($search, $string) {
		...
		// enl�ve balises inutiles devant-derri�re //
...
?>
		<h2><?php echo $nb_results." ".eText::style_to_html($traductions['r�sultats']." ".$traductions['pour']." : ".$search_string); ?></h2>
<?php
...
?>
			<div class="search_result">
				...
				<br /><br />
				<small><?php echo ucfirst($traductions['mots cl�s']); ?> : <?php echo $resultats[$i]['keywords']; ?></small>
			</div>
<?php			
		} // END OF FOR
