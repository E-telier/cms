<?php
	...
	$rq_parent = "SELECT childof FROM ".$localized_sql_prefix."_cms_pages WHERE childof<>reference AND reference='".$page_datas['childof']."';";
	...
	// Parent Page //
	$rq_subsection = "SELECT reference FROM ".$localized_sql_prefix."_cms_pages WHERE reference='".$subsection."' LIMIT 1;";
	$result_subsection = mysqli_query($mysqli, $rq_subsection);
	$content_subsection = mysqli_fetch_array($result_subsection);	
	/*
?>
										<!-- <li><a href="<?php echo $folderURL.$content_subsection['reference']; ?>"><?php echo htmlentities('Pr�sentation'); ?></a></li>-->
<?php
	*/
	
	
	// Children //
	$rq_children = "SELECT reference, menu_name FROM ".$localized_sql_prefix."_cms_pages WHERE childof<>reference AND childof='".$subsection."' AND menu_position>0 ORDER BY menu_position ASC, menu_name ASC;";
	//echo $rq_children;
...
?>
										<li class="<?php echo  $content_children['reference']; ?>"><a href="<?php echo $folderURL.$content_children['reference']; ?>" class="<?php echo $class; ?>"><?php echo eText::iso_htmlentities($content_children['menu_name']); ?></a></li>
<?php

		$rq_sub = "SELECT reference, menu_name FROM ".$localized_sql_prefix."_cms_pages WHERE childof<>reference AND childof='".$content_children['reference']."' AND menu_position>0 ORDER BY menu_position ASC, menu_name ASC;";
