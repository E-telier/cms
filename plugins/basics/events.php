...
?>
									...
									<!--
										...
										$(document).ready(function() {
											$('head').append('<link rel="stylesheet" href="'+websiteRootURL+'plugins/events.css?d=201405071218" type="text/css" />');
										});
									-->
									</script>
<?php
	if (isset($currentDatas[1])) {
		// SHOW CURRENT EVENTS //
		$rq = "SELECT * FROM ".eParams::$prefix.eLang::get_lang()."_back_events WHERE title='".addslashes(urldecode($currentDatas[1]))."';";		
		$result_datas = eMain::$sql->sql_to_array($rq);
		$content = $result_datas['datas'][0];
		
		$rq = "UPDATE ".eParams::$prefix.eLang::get_lang()."_back_events SET views=views+1 WHERE id=".$content['id'].";";
		//echo $rq;
...
<?php
		...
		if (isset($_POST['date_from'])) {
			...
			$rq = "SELECT date_from FROM ".eParams::$prefix.eLang::get_lang()."_back_events ORDER BY date_from ASC LIMIT 1;";
			$result_datas = eMain::$sql->sql_to_array($rq);
			$content = $result_datas['datas'][0];
...
<?php	
		...
		$rq = "SELECT * FROM ".eParams::$prefix.eLang::get_lang()."_back_events $sql_clauses ORDER BY date_from ASC, date_to DESC LIMIT ".$limit.";";
		//echo $rq;
		$result_datas = eMain::$sql->sql_to_array($rq);
